...
class UserTableList extends Component
{
    ...
    public $user_id, $firstname, $lastname, $email,$username,$password,$user_edit_id, $user_delete_id;
    public $location_code;
    public $UserRole;
    // public $UserRole = ['value' => null ];

    /**
     * Modal Variables
     */
    public $showUserEdit = false;
    public $showUserAssign = false;
    ...
    public function storeUserData() 
    {
        try {
            ...
            $user->location_code = $this->location_code;
            ...
            $this->notification([
                'icon'          => 'success',
                'title'         => 'User added!',
                'description'   => '<strong>'. $user->firstname . '</strong> has been added successfully.'
            ]);
    ...
    public function editUser($id)
    {
        ...
        // If location_code exists on the user model, assign its value
        if (property_exists($user, 'location_code')) {
            $this->location_code = $user->location_code;
        }
    }
    ...
    public function editUserData()
    {
        try {
            ...
            if (!empty($this->password)) {
                ...
                $user->password = null; // Set password to null if not provided
            }
    ...
    public function confirmDelete()
    {
        ...
        if ($user) {
            ...
            session()->flash('message', 'User Credential has been deleted successfully');
        }
    ...
    /**
     * Location Code drop-down filter
     */
    public function selectedOutletFilters()
    { 
    ...
    public function RoleFilters()
    {
        $this->roleFilter = [
            ...
            [
                'id' => '3',
                'name' => 'Promo Merchandiser',
                'value' => 'Promo Merchandiser',
            ],
            [
                 'id' =>'4',
                 'name' => 'Team Leader',
     ...
     * Reset Button Filter 
     */
    public function resetDataFilter()
    {
     ...
     /**
     *Mount file for Filter method 
     */
     public function mount()
     {
        $this->selectedOutletFilters();
    ...
    public function render()
    {
     ...
     //    $outletFilters = $this->selectedOutlet;
        ...
        ->when($roleFilter, function ($query) use ($roleFilter) {
            $query->where('UserRole', 'like', '%' . $roleFilter . '%');
        })
        ->when($letterFilter, function ($query) use ($letterFilter) {
            $query->where(function ($subQuery) use ($letterFilter) {
                $subQuery->where('firstname', 'like', $letterFilter . '%');
            });
        })
        ->when($selectedOutlet, function ($query) use ($selectedOutlet) {
          $query->where('location_code', 'like', '%' . $selectedOutlet . '%');
        });

        $pagination = $users->orderBy($this->sortBy, $this->sortDirection)
               ->paginate(5);
        
        $users = $users->orderBy($this->sortBy, $this->sortDirection)
               ->get();
               
        return view('livewire.user-table-list', compact('users','pagination'));
    }
