...
class SalesDataComponent extends Component
{
    use WithPagination;
    ...
    public $confirmingExport = true;

    public $startDate; 
    public $endDate; // filter using dates

    public $isDownloadingFile;

    //Export Data
    public function submitExportData()
    {
        $this->validate([
     ...
     //    $search = '%'.$this->searchTerm.'%';
        ...
        $getDataWithinDates = SMSApi::when($this->startDate || $this->endDate || $brandFilter || $outletFilters || $letterFilter || 
        $search, function ($query) use ($brandFilter, $outletFilters, $letterFilter, $search) {
          $query = $query->betweenDates('created_at', Carbon::parse($this->startDate)->format('Y-m-d\TH:i'), Carbon::parse($this->endDate));
          if ($brandFilter) {
              $query = $query->where('brand', $brandFilter);
          }
          if ($outletFilters) {
              $query = $query->where('outlet', $outletFilters);
          }
          if ($letterFilter) {
              $query = $query->where(function ($subQuery) use ($letterFilter) {
                  $subQuery->where('firstname', 'like', $letterFilter.'%')
        ...
        // dd($getDataWithinDates);
        
        $this->isDownloadingFile = true;
        
        $filename = time() . '_sales_data.csv';
        
        $quantity_total = $getDataWithinDates->sum('total_quantity');
        
        $getDataWithinDates->push(['', '', '', '', '', '', '', '', 'Total Quantity: ', $quantity_total]);
        
        (new FastExcel($getDataWithinDates))->export(storage_path('app/public/' . $filename));
    ...
    {
        $data = SmsApi::whereBetween('created_at', [date($this->startDate), date($this->endDate)])
            ->paginate(5);
    ...
    public function mount()
    {
        ...
        $this->outletFilters = collect($locationCodes)->map(function ($value, $key) {
            return [
                'value' => $value, // Change this line to use the $value instead of $key
                'name' => $value, // Also, use the $value here
            ];
        })->toArray();
    }
    ...
    public function render()
    {
     $search = '%'.$this->searchTerm.'%';
     $brandFilter = '%'.$this->selectedFilter.'%';
     $letterFilter = $this->selectedLetter; // Assuming you have a property named $selectedLetter for the selected letter filter
     // $outletFilters = '%'.$this->selectedOutlet.'%';
     $selectedOutlet = $this->selectedOutlet;

     $smsApiGet = SMSApi::where('brand', 'like', $brandFilter)
     ->when($selectedOutlet, function ($query) use ($selectedOutlet) {
          $query->where('outlet', 'like', '%' . $selectedOutlet . '%');
     })
     ->when($letterFilter, function ($query) use ($letterFilter) {
     return $query->where(function ($subQuery) use ($letterFilter) {
          $subQuery->where('firstname', 'like', $letterFilter.'%');
               // ->orWhere('lastname', 'like', $letterFilter.'%');
     });
     })
     ...
     return $query->betweenDates('created_at', Carbon::parse($this->startDate)->format('Y-m-d\TH:i'), Carbon::parse($this->endDate));
     });

     
     $smsApi = $smsApiGet->orderBy($this->sortBy, $this->sortDirection)
          ->get();

     $smsPaginate = $smsApiGet->orderBy($this->sortBy, $this->sortDirection)
          ->paginate(5);

     $rtotal = 0;
     foreach($smsApi as $itemCode){
          $rtotal += $itemCode->unit_price;
     }
     $formattedTotal = '₱' . number_format($rtotal, 2, '.', ',');

     $quantity = 0;
     foreach($smsApi as $itemCode){
          $quantity += $itemCode->total_quantity;
     }

     return view('livewire.sales-data-component', ['outletFilters' => $this->selectedOutlet],
          compact('smsApi', 'rtotal','smsPaginate','quantity','formattedTotal'));        
   }
