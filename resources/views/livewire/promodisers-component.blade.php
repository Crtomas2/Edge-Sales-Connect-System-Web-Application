                    ...
                    <table class="table-auto overflow-scroll w-full max-w-full px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                        <thead class="px-4 py-3 bg-gray-200 text-right sm:px-6 border-b sm:rounded-bl-md sm:rounded-br-md">
                            <tr height="50">
                                ...
                                <th role="button" align="center" wire:click="setSort('promodiser_status', '{{ $sortBy === 'promodiser_status' && $sortDirection === 'ASC' ? 'DESC' : 'ASC' }}')">
                                    <div class="flex items-center justify-center space-x-4 py-2 pl-8 pr-4">
                                        <span>Status</span>
                                        @if($sortBy)
                                            @if($sortBy === 'promodiser_status')
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="height: 16px; width: 16px;">
                                                    @if($sortDirection === 'ASC')
                                                        <path fill-rule="evenodd" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L6.707 7.707a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                                    @else
                        ...
                        <tbody class="divide-y">
                            @if($promodisers->count() > 0)
                                ...
                                <tr>
                                    ...
                                    <td align="center" class="border-b border-slate-200 p-4 pl-8">{{ $promodiser->Lastname}}</td>
                                    <td align="center" class="border-b border-slate-200 p-4 pl-8">{{ $promodiser->Mobilenumber}}</td>
                                    <td align="center" class="border-b border-slate-200 p-4 pl-8">{{ $promodiser->latest_assignment ? $promodiser->latest_assignment->location->LocationCode : 'none' }}</td>
                                    <td align="center" class="border-b border-slate-200 p-4 pl-8">{{ $promodiser->promodiser_status}}</td>
                                    <td align="right">
                                        <div class="flex px-4">
    ...
    <x-jet-dialog-modal wire:model="showPromodiserCreate">
        ...
        <x-slot name="content">
            <form wire:submit="addPromodiserData()">
                ...
                <div class="w-full mb-3">
                    <x-jet-label for="promodiser_status">Status</x-jet-label>
                    <x-jet-input type="text" class="w-full mb-2" id="promodiser_status" wire:model.defer="promodiser_status" size="12" />
                    <x-jet-input-error for="promodiser_status" />
                </div>
            </form>
        </x-slot>
    ...
    <x-jet-dialog-modal wire:model="showPromodiserEdit">
        ...
        <x-slot name="content">
            <form wire:submit="editPromodiserData()">
                ...
                <div class="w-full mb-3">
                    <x-jet-label for="promodiser_status">Mobile number</x-jet-label>
                    <x-jet-input type="text" class="w-full mb-2" id="mobile_number" wire:model.defer="Mobilenumber" size="12" />
                    <x-jet-input-error for="Mobilenumber" />
                </div>
                <div class="w-full mb-3">
